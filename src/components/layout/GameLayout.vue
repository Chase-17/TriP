...
// Импортируем interactionStore для отслеживания состояния панелей
...
// Wake Lock - не дать экрану погаснуть (из userPrefsStore)
...
// Восстанавливаем wake lock после первого взаимодействия
...
// Начать новый ход (восстановить движение всем)
const startNewTurn = () => {
  charactersStore.startNewTurn()
  // TODO: синхронизировать с игроками через сессию
  console.log('🔄 Новый ход! Движение и порывы восстановлены.')
}

// Порывы выбранного персонажа
...
const useSurge = () => {
  ...
  if (result.success) {
    console.log(`⚡ Порыв! +${result.movementGained} ОД`)
  } else {
    console.log('❌ Нет доступных порывов')
  }
}
      ...
      <template v-if="activeScreen === 'battle-map'">
        ...
        <template v-else>
          <div class="action-buttons">
            ...
            <!-- Кнопка "Новый ход" (временная, для тестирования) -->
            <button 
              class="action-btn new-turn-btn"
              @click="startNewTurn"
            >
              <Icon icon="mdi:refresh" class="btn-icon" />
              <span class="btn-label">Новый ход</span>
            </button>
            
            <!-- Кнопка "Порыв" - тратит порыв для получения доп. движения -->
            <button 
            ...
            >
              ...
              <span class="btn-label">Порыв ({{ selectedCharacterSurges }})</span>
            </button>
          </div>
        </template>
      </template>
    ...
    <Transition name="menu-fade">
      <div v-if="menuOpen" class="menu-overlay" @click="menuOpen = false">
        <div class="menu-content" @click.stop>
          ...
          <!-- Wake Lock - не дать экрану погаснуть -->
          <div class="menu-section">
            <button 
            ...
            >
              ...
              <span class="wake-lock-status">{{ keepScreenAwake ? 'ВКЛ' : 'ВЫКЛ' }}</span>
            </button>
          </div>
