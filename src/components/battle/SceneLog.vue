...
import { getPortraitUrl } from '@/utils/rendering/tokenRenderer'
...
const { 
  ...
  remainingEventsCount = ref(0),
  totalEventsCount = ref(0),
  currentImage = ref(null), 
  hasActiveImage = ref(false), 
  activeFilter = ref('all'), 
  perspectiveUserId = ref(null) 
} = safeStoreToRefs(sceneLogStore, 'sceneLog')
const { role = ref('player'), connections = ref([]) } = safeStoreToRefs(sessionStore, 'session')
