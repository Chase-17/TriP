...
import EquipmentSelector from '../equipment/EquipmentSelector.vue'
...
// Panel expansion state - для шагов 4+
const panelExpanded = ref(false)
const infoPanelRef = ref(null)

// Шаги 5 и 6 всегда в fullscreen режиме (нет графической схемы)
const isFullscreenStep = computed(() => step.value >= 5)

// Развёрнут ли info panel на весь экран
const isPanelFullscreen = computed(() => isFullscreenStep.value || panelExpanded.value)
...
const canProceed = computed(() => {
  switch (step.value) {
    ...
    case 6: {
      // Need name (2+ chars) and portrait
      return formData.value.name.trim().length >= 2 && formData.value.portrait !== null
    }
...
// === Panel expansion logic (шаг 4) ===
// Порог скролла для развёртывания панели (в пикселях)
const SCROLL_EXPAND_THRESHOLD = 100

// При смене шага сбрасываем expanded состояние
watch(step, () => {
  panelExpanded.value = false
})

// Обработчик скролла для автоматического развёртывания
const handlePanelScroll = (event) => {
  // Только для шага 4 (класс + навыки)
  if (step.value !== 4 || isFullscreenStep.value) return
  
  const scrollTop = event.target.scrollTop
  
  // Если проскроллили достаточно глубоко — разворачиваем
  if (scrollTop > SCROLL_EXPAND_THRESHOLD && !panelExpanded.value) {
    panelExpanded.value = true
  }
  // Если вернулись наверх — сворачиваем
  else if (scrollTop <= 20 && panelExpanded.value) {
    panelExpanded.value = false
  }
}

// Свернуть панель и показать схему (для кнопки "Другой класс")
const collapsePanel = () => {
  panelExpanded.value = false
  // Скроллим панель наверх
...
<template>
  <div class="fixed inset-0 z-50 flex flex-col lg:flex-row bg-slate-950 select-none">
    <!-- Main Canvas Area - Animated height transition -->
    <div 
      class="canvas-container relative overflow-hidden touch-none bg-slate-950 flex-shrink-0 transition-all duration-500 ease-out"
      :class="{ 
        'canvas-collapsed': isPanelFullscreen,
        'canvas-expanded': !isPanelFullscreen 
      }"
      @wheel="handleWheel"
    ...
    <!-- When fullscreen: takes entire screen -->
    <div 
      class="bg-slate-900 border-t lg:border-t-0 lg:border-l border-white/10 flex flex-col relative overflow-hidden transition-all duration-300"
      :class="isPanelFullscreen ? 'flex-1 lg:flex-none lg:w-full' : 'flex-1'"
    >
      ...
      <div class="sticky top-0 z-20 bg-slate-900/95 backdrop-blur-sm px-6 py-4 border-b border-slate-700/50">
        <div class="flex items-center justify-between">
          <!-- Кнопка "Назад" или "Другой класс" в expanded режиме -->
          <button
            v-if="step > 1"
            @click="panelExpanded && step === 4 ? collapsePanel() : prevStep()"
          ...
          >
            ...
            <span class="text-sm font-medium">
              {{ panelExpanded && step === 4 ? 'Другой класс' : 'Назад' }}
            </span>
          </button>
          <div v-else></div>
      ...
      >
        <!-- Title (skip for step 4, 5, 6 as they have custom layout) -->
        <div v-if="step <= 3" class="mb-4">
          ...
          <p class="text-sm text-slate-400 mt-1">
            {{ step === 1 ? 'Выберите пол персонажа для начала' :
               ...
               'Подраса добавляет связь с одним из аспектов' }}
          </p>
        </div>
...
/* Canvas expanded state (normal) */
.canvas-expanded {
  max-height: 100vh;
  opacity: 1;
}

/* Canvas collapsed state (panel is fullscreen) */
.canvas-collapsed {
  max-height: 0;
  opacity: 0;
  padding: 0;
