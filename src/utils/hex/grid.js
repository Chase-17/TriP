...
export class HexGrid {
  ...
  /**
   * Получить все гексы в радиусе от центра
   * @param {number} centerQ - координата Q центра
   * @param {number} centerR - координата R центра
   * @param {number} radius - радиус (1 = только центр, 2 = центр + соседи, и т.д.)
   * @returns {Array<{q: number, r: number}>} массив гексов
   */
  getHexesInRadius(centerQ, centerR, radius) {
    const hexes = []
    const actualRadius = radius - 1 // radius=1 означает только центр
    
    for (let dq = -actualRadius; dq <= actualRadius; dq++) {
      for (let dr = Math.max(-actualRadius, -dq - actualRadius); dr <= Math.min(actualRadius, -dq + actualRadius); dr++) {
...
/**
 * Расстояние между двумя гексами (standalone функция)
 * @param {number} q1 - q первого гекса
 * @param {number} r1 - r первого гекса
 * @param {number} q2 - q второго гекса
 * @param {number} r2 - r второго гекса
