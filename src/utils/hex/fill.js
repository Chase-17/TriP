...
import { HEX_DIRECTIONS, hexKey, parseHexKey } from './grid'
...
/**
 * Получить соседей гекса (использует общие направления из grid.js)
 */
 ...
 * @param {boolean} probabilistic - Вероятностный режим (для кисти)
 * @returns {Set<string>} - Выбранные ключи гексов
 */
function selectHexesWithClustering(candidateKeys, density, clustering, rng, probabilistic = false) {
  if (candidateKeys.length === 0) return new Set()
  
  const selected = new Set()
  
  // Вероятностный режим: каждый гекс имеет шанс density% быть выбранным
  // Это даёт правильное статистическое распределение при многократных мазках
  if (probabilistic) {
    const probability = density / 100
    
    if (clustering <= 10) {
      // Без кластеризации - простой рандом для каждого гекса
      ...
      // С кластеризацией - сначала выбираем "семена", потом растим от них
      const neighborBias = clustering / 100
      const candidates = new Set(candidateKeys)
      
      // Проходим по всем гексам и с вероятностью делаем их семенами
      const shuffled = rng.shuffle([...candidateKeys])
      for (const key of shuffled) {
        if (!candidates.has(key)) continue
        
        // Шанс стать семенем = density * (1 - clustering * 0.5)
        // Чем выше кластеризация, тем меньше семян, но они крупнее
        const seedProbability = probability * (1 - neighborBias * 0.5)
        
        if (rng.next() < seedProbability) {
          selected.add(key)
          candidates.delete(key)
          
          // Растим кластер от семени
          const clusterSize = Math.max(1, Math.round(1 / (1 - neighborBias * 0.8)))
          let grown = 0
          const toGrow = [key]
          
          while (toGrow.length > 0 && grown < clusterSize) {
            const currentKey = toGrow.shift()
            const { q, r } = parseHexKey(currentKey)
            const neighbors = rng.shuffle(getHexNeighbors(q, r))
            
            for (const neighbor of neighbors) {
              const neighborKey = hexKey(neighbor.q, neighbor.r)
              if (candidates.has(neighborKey) && rng.next() < neighborBias) {
                selected.add(neighborKey)
                candidates.delete(neighborKey)
                toGrow.push(neighborKey)
                grown++
                if (grown >= clusterSize) break
              }
            }
          }
        }
  ...
  // Детерминированный режим (для заливки выделенной области)
  ...
  while (selected.size < targetCount && candidates.size > 0) {
    ...
    if (useNeighbor) {
      ...
      for (const selectedKey of shuffledSelected) {
        const { q, r } = parseHexKey(selectedKey)
 ...
 * @param {boolean} options.probabilistic - Вероятностный режим (для кисти, даёт правильное распределение при многократных мазках)
...
export function applyFillProfile(profile, hexKeys, terrainStore, options = {}) {
  const { previewOnly = false, customSeed = null, probabilistic = false } = options
  ...
  // Шаг 1: Заполняем все гексы базовым террейном (если он указан)
  if (profile.baseTerrain) {
